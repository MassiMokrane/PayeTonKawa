describe(" Order Service - Tests PayeTonKawa", () => {
  ...
  test("✅ Calcul total commande - Cas simple", () => {
    const orderItems = [
      { unitPrice: 10.5, quantity: 2, totalPrice: 21.0 },
      { unitPrice: 15.0, quantity: 1, totalPrice: 15.0 },
  ...
  test("✅ Calcul total OrderItem - Prix unitaire × quantité", () => {
    ...
    expect(calculateItemTotal(12.5, 2)).toBe(25.0);
    expect(calculateItemTotal(19.99, 3)).toBe(59.97);
    expect(calculateItemTotal(5.25, 4)).toBe(21.0);
    expect(calculateItemTotal(100, 1)).toBe(100);
    expect(calculateItemTotal(0.5, 10)).toBe(5.0);
  });

  test("✅ Calcul total complexe - Multiples produits", () => {
    const orderItems = [
      { productId: 1, name: "Café Arabica", unitPrice: 12.5, quantity: 2 },
      { productId: 2, name: "Café Colombia", unitPrice: 18.0, quantity: 1 },
      { productId: 3, name: "Café Brazil", unitPrice: 10.0, quantity: 3 },
      { productId: 4, name: "Café Ethiopia", unitPrice: 22.5, quantity: 1 },
    ];
    ...
    // Vérifications
    expect(itemsWithTotals[0].totalPrice).toBe(25.0);
    expect(itemsWithTotals[1].totalPrice).toBe(18.0);
    expect(itemsWithTotals[2].totalPrice).toBe(30.0);
    expect(itemsWithTotals[3].totalPrice).toBe(22.5);
    expect(totalOrder).toBe(95.5);
  });
  ...
  test("✅ RabbitMQ - Format message STOCK_UPDATED", () => {
    ...
    expect(stockUpdateMessage.data.oldQuantity).toBeGreaterThan(
      stockUpdateMessage.data.newQuantity
    );
  });
  ...
  test("✅ RabbitMQ - Publication message queue", () => {
    ...
    const message = {
      type: "ORDER_CREATED",
      data: { orderId: 123, userId: 456, total: 75.5 },
    };
  ...
  test("✅ Prometheus - Métriques HTTP requests", () => {
    ...
    expect(httpMetrics.http_request_duration_seconds.duration).toBeGreaterThan(
      0
    );
  });
  ...
  test("✅ Utilitaires - Formatage prix", () => {
    ...
    expect(formatPrice(12.504)).toBe(12.5);
  ...
  test("✅ Validation format email pour notifications", () => {
    ...
    const invalidEmails = ["invalid-email", "@domain.com", "email@", ""];
